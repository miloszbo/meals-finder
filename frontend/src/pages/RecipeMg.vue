  ...
  <label class="label"><span class="label-text">Ingredients</span></label>
      ...
      <div class="flex gap-4 mb-4">
        <div class="form-control w-1/2">
          ...
          <input type="number" v-model="form.time" placeholder="e.g. 45 min" class="input input-bordered w-full" />
        </div>
        <div class="form-control w-1/2">
...
import { addRecipe, getTags } from '@/api/axios'
...
function addTag(tagName, categoryName) {
  if (!selectedTags.value.find(tag => tag.name === tagName && tag.type === categoryName)) {
    selectedTags.value.push({ name: tagName, type: categoryName })
...
const submitRecipe = async () => {
  ...
  recipe: form.value.description,
  tags: JSON.parse(JSON.stringify(selectedTags.value.map(tag => ({ name: tag.name, type: tag.type })))),
  ingredients: {"ingredients":JSON.parse(JSON.stringify(ingredients.value))}
  }
  if (form.value.image) {
