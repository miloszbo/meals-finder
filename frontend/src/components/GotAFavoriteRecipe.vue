<template>
  <!-- Section for submitting a favorite recipe -->
  <section class="flex flex-col items-center justify-center mt-8 mb-8">
    <!-- Section title -->
    <h2 class="text-3xl font-bold mb-2 text-white">Got a favorite recipe?</h2>
    <!-- Section subtitle -->
    <p class="text-gray-300 mb-6">Share it with community<span class="font-bold">.</span></p>
    <!-- Submit button as router-link -->
    <router-link
      to="/recipemg"
      class="px-6 py-2 rounded bg-[#171717] text-white font-medium hover:bg-[#5046b8] transition text-center"
    >
      Submit it here
    </router-link>
  </section>
</template>
